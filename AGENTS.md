...
이 파일은 짧은 bootstrap map이다. 공통 실행 계약과 jjcat 상세 운영 규칙은
repository-owned 문서가 소유한다.

## First Read

- 공통 하네스 인터페이스와 product/remote overlay: `docs/agent-harness.md`.
...
- 구현 여부는 code/test/user-visible smoke가 기준이며 planned 기능을 implemented로 표시하지 않는다.
- local과 SSH transport는 같은 domain contract를 사용하되 credential과 host inventory는 저장소에 기록하지 않는다.
- remote command는 shell 문자열 결합이 아니라 구조화된 argv/stdin 경계를 사용한다.
- 이 저장소는 public tracked surface다. prompt, 대화 transcript, memory, raw tool output,
  machine-local inventory는 작업 근거로 사용할 수 있지만 tracked artifact로 옮기지 않는다.
- AI-assisted 변경도 사람의 변경과 동일한 acceptance, test, review와 provenance 기준을
  통과해야 하며 model 응답 자체를 구현 또는 검증 evidence로 간주하지 않는다.
- 기본 전체 검증은 `scripts/check.sh`; publication 경계는 `scripts/check-publication-boundary.py`로 확인한다.
- 로컬 VCS는 `jj`를 사용하고 push, remote 생성, license, package publish와 release는
  명시적 결정이 있을 때만 수행한다.

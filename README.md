...
Pre-alpha P0 read-only cockpit이 동작한다. Tauri desktop shell에서 local/SSH 저장소를
등록하고 탭으로 전환하며, versioned cache와 `jj` change projection을 조회할 수 있다.

현재 작업은 [P1 Multi-repository Cockpit](docs/todo-p1-multi-repository-cockpit/spec.md)이다.
...
아직 remote origin과 license는 설정하지 않았다. remote는 private 또는 public으로 시작할
수 있지만 content 기준은 바뀌지 않는다. `LICENSE`가 추가되기 전까지 특정 open-source
license가 적용된다고 가정하지 않는다.
...
원격 생성, push, license 선택은 별도 사용자 결정과 publication gate를 요구한다.
